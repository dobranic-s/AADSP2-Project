...
DSPfract IIR_low_pass_1000Hz_2nd_order[6] = { 0.00392209866199085940/2, 0.00784419732398171890/2, 0.00392209866199085940/2,
													 1.00000000000000000000/2, -1.81534108270456800000/2, 0.83100558934675750000/2 };


DSPfract IIR_high_pass_500Hz_2nd_order[6] = { 0.95466161616884615000, -1.90932323233769230000, 0.95466161616884615000,
													 1.00000000000000000000, -1.90750162605361280000, 0.91159449660707459000};


static DSPfract l_x_history[2];
static DSPfract l_y_history[2];
static DSPfract r_x_history[2];
static DSPfract r_y_history[2];
...
{	
	...
	if (mode == OM_2_2_0)	// mode1 = 0 i mode2 = 0
	{
		for (DSPint i = 0; i < BLOCK_SIZE; i++)
		{
			...
			*leftChannelOut = second_order_IIR((*leftSurroundChannelOut), IIR_high_pass_500Hz_2nd_order, l_x_history, l_y_history);	// L
